        ...
        <section class="md:col-span-3">
            ...
            <div class="container mx-auto p-4">
                ...
                <div x-data="{ open: false }">
                    ...
                    <div x-cloak x-show="open" class="fixed inset-0 flex items-center justify-center
                    bg-gray-900 bg-opacity-50">
                        <div @click.away="open=false" class="bg-white p-6 rounded-lg shadow-md w-full max-w-md">
                            ...
                            <form method="POST" action="{{route('applicant.store', $job->id)}}"
                                ...
                                <button @click="open=false" class="bg-gray-300 hover:bg-gray-400 text-black px-4 py-2
                                rounded-md">
                                    Cancel
                                </button>
                            </form>
                ...
                @else
                    <p class="my-5 bg-gray-200 text-black rounded-xl p-3">
        ...
        <aside class="bg-white rounded-lg shadow-md p-3">
            ...
            @guest
                <p class="mt-10 bg-gray-200 text-gray-700 font-bold w-full py-2 px-4 rounded-full text-center">
                    <i class="fas fa-info-circle mr-3"></i>You have to be logged in to bookmark this job!
                </p>
            @else
                @if(auth()->user()->bookmarkedJobs()->where('job_id',$job->id)->
                exists())
                    <form method="POST" action="{{route('bookmarks.destroy', $job->id)}}" class="mt-4">
                        @csrf
                        @method('DELETE')
                        <button class="mt-10 bg-red-500 hover:bg-red-600 text-white font-bold
                 w-full py-2 px-4 rounded-full flex items-center justify-center"
                        ><i class="fas fa-bookmark mr-3"></i> Remove Bookmark</button>
                    </form>
                @else
                <form method="POST" action="{{route('bookmarks.store', $job->id)}}" class="mt-4">
                    @csrf
                    <button class="mt-10 bg-blue-500 hover:bg-blue-600 text-white font-bold
                 w-full py-2 px-4 rounded-full flex items-center justify-center"
                ><i class="fas fa-bookmark mr-3"></i> Bookmark Listing</button>
                </form>
                @endif
            @endguest
        </aside>
